...
## Amendment 2026-06-20 (T9.2 Option 2) -- composite records (subtree-inclusive), SEPARATE file

RATIFIED 2026-06-20 by Sudnya (after Cowork verification: separate composite file 76
records / 20 routines with zero key-overlap; frozen exclusive defaults preserved;
not-a-superset confirmed at full 3085 = 8 hazard drops + 5 content-changes). The committed
`traces/test-plans/*.jsonl` stay FROZEN and EXCLUSIVE (leaf-first). To give non-leaf COMPOSITE routines a hermetic bench bar, a
separate plan file `traces/test-plans/composites-inclusive.jsonl` is generated by
`tools/gen_composite_plan.js` from inclusive (subtree-folded) heavy traces
(heavy-trace.md note #4 amendment). Record FORMAT is unchanged -- same fields, same
self-validation contract; the only difference is that a composite record's `reads`/
`writes` are its full subtree closure (callees inline), so the existing bench replays it
unchanged (it executes the callees inline and matches).

- SEPARATE, not merged: inclusive is NOT a strict superset of exclusive (it drops
  hazard-subtree records and content-changes a few -- see heavy-trace.md note #4). The
  composite file therefore contains ONLY records whose `(entry_pc, path_id)` is ABSENT
  from every committed exclusive plan (path_id is attribution-independent). This adds the
  new composite bodies (0x1553, 0x2a40, 0x15a0, ...) without disturbing any frozen record.
- The bench consumes BOTH: committed exclusive plans (leaves) + the composite file
  (composites). A composite port is validated by this hermetic record AND by live
  transparency.
- Regeneration is memory-heavy: run with raised Node heap (NODE_OPTIONS=
  --max-old-space-size=4096) and/or capped frames.
